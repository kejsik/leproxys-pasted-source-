...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	do {
		...
		Log(L" Driver: Image base allocated at 0x" << reinterpret_cast<void*>(kernel_image_base) << std::endl);
		...
		if (destroyHeader) {
			...
			Log(L" Passed 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header" << std::endl);
		}
		...
		Log(L" Entry 0x" << reinterpret_cast<void*>(address_of_entry_point) << std::endl);
		...
		Log(L" Entry returned at 0x" << std::hex << status << std::endl);
