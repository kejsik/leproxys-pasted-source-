...
inline void crash(const char* text) {
	std::cout << xorstr("error : ") << text << std::endl;
...
{
	//if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("Window"), NULL)) BlackList();
	//if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("Qt5QWindowIcon"), NULL)) BlackList();
	//if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("ProcessHacker"), NULL)) BlackList(); 
	if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("PROCEXPL").c_str(), NULL)) BlackList();
	if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("dbgviewClass").c_str(), NULL)) BlackList();
	if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("XTPMainFrame").c_str(), NULL)) BlackList();
	if (LI_FN(FindWindowA).forwarded_safe_cached()(xorstr("WdcWindow").c_str(), xorstr("Resource Monitor").c_str())) BlackList();
}
void antidump()
{
	DWORD Protect = 0;
...
void security_loop()
{
	...
	while (true)
	{
		...
		if (is_debugger_present()) BlackList();
