...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 col_bg;
    col_bg = ImGui::GetColorU32(ImVec4(0.63f, 0.00f, 0.96f, it_anim->second.animation));
    ...
    //Circle anim
    if (*v)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, ImColor(1.0f, 1.0f, 1.0f, it_anim->second.animation), 30);
    else
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0])
            ClearActiveID();
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard && g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImColor(161, 0, 245));
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 6, ImColor(255, 255, 255));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        ImGui::PushFont(colorpicker);

        ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(col_source));

        RenderText(ImVec2(bb_inner.Max.x - 22, bb_inner.Min.y + 0), "G");
